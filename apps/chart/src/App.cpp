// Based on the ImPlot demo application framework by Evan Pezent:
...
App::App(std::string title, int w, int h, int argc, char const* argv[]) {
    ...
    CLI::App options{title};
    options.set_help_flag("--help", "Show help");
    options.add_flag("--no-vsync", no_vsync, "Disable vertical synchronization");
    options.add_flag("-m,--msaa", use_msaa, "Enable 4x MSAA");
    options.add_flag("-i,--imgui-style", im_style, "Use the default ImGui/ImPlot dark style");
    options.add_option("-w,--width", w, "Window width")->check(CLI::PositiveNumber);
    options.add_option("-h,--height", h, "Window height")->check(CLI::PositiveNumber);
    options.add_flag("-g,--gpu", UsingDGPU, "Request the discrete GPU on hybrid laptops");
    options.add_option("--font", font_path, "TTF/OTF font path");
    options.add_option("--font-size", font_size, "Font size in pixels")->check(CLI::Range(8.0f, 48.0f));
    ...
    try {
        ...
        std::exit(options.exit(error));
    }

#if defined(_WIN32)
    NvOptimusEnablement = AmdPowerXpressRequestHighPerformance = UsingDGPU ? 1UL : 0UL;
#endif
...
#if defined(_WIN32)
    if (font_path.empty()) {
        const std::filesystem::path segoe_ui = "C:/Windows/Fonts/segoeui.ttf";
        if (std::filesystem::exists(segoe_ui))
            font_path = segoe_ui.string();
    }
#endif
    if (!font_path.empty() && io.Fonts->AddFontFromFileTTF(font_path.c_str(), font_size) == nullptr) {
